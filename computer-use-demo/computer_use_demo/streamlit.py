...
if os.getenv("WANDB_API_KEY"):
    ...
    raise RuntimeError("WANDB_API_KEY is not set in the environment variables.")
...
def setup_state():
    ...
    if "wandb_api_key" not in st.session_state:
        # Load WANDB_API_KEY from storage or environment variable
        st.session_state.wandb_api_key = load_from_storage("wandb_api_key") or os.getenv(
            "WANDB_API_KEY", ""
        )
    # Set the WANDB_API_KEY in the environment variables
    os.environ["WANDB_API_KEY"] = st.session_state.wandb_api_key
...
async def main():
    ...
    if st.session_state.wandb_api_key:
        wandb.login(key=st.session_state.wandb_api_key)
        weave.init("morgan/computer-use")
    else:
        raise RuntimeError("WANDB_API_KEY is not set in the environment variables.")
    ...
    with st.sidebar:
        ...
        st.text_input(
            ...
            type="password",
            key="wandb_api_key",
            on_change=lambda: save_to_storage("wandb_api_key", st.session_state.wandb_api_key),
        )
